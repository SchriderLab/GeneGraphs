...
def prob_a001_coal_(i, a0, a1, m, t0, t1, N, n1, n2):
    t = t1 - t0
...
def prob_a001_mig_(a0, a1, m, t0, t1, N, n1, n2):
    t = t1 - t0
...
def prob_a001_coal(i, a0, a1, m, t0, t1, N, n1, n2):
    ...
    if i == 0:
        ...
        if n1 > 1:
            return f_a001_coal(a1, N * np.exp(-a1 * t0), t) / F_a001_coal(a1, N * np.exp(-a1 * t0), t) * (1 - F_a001_coal(a1, N * np.exp(-a1 * t0), t)) ** (nC2(n2) - 1) * (1 - F_a001_coal(a0, N * np.exp(-a0 * t0), t)) ** (nC2(n1)) * (1 - F_a001_mig(a0, a1, m, N * np.exp(-a0 * t0),  N * np.exp(-a1 * t0), t)) ** n1
        elif n1 == 1:
            return f_a001_coal(a1, N * np.exp(-a1 * t0), t) / F_a001_coal(a1, N * np.exp(-a1 * t0), t) * (1 - F_a001_coal(a1, N * np.exp(-a1 * t0), t)) ** (nC2(n2) - 1) * (1 - F_a001_mig(a0, a1, m, N * np.exp(-a0 * t0), N * np.exp(-a1 * t0), t)) ** n1
        else:
            if n2 > 2:
                ret = f_a001_coal(a1, N * np.exp(-a1 * t0), t) / F_a001_coal(a1, N * np.exp(-a1 * t0), t) * (1 - F_a001_coal(a1, N * np.exp(-a1 * t0), t)) ** (nC2(n2) - 1)
                return ret
            
            else:
                ret = f_a001_coal(a1, N * np.exp(-a1 * t0), t)
                return ret
...
def prob_a001_mig(a0, a1, m, t0, t1, N, n1, n2):
    ...
    if n2 >= 2 and n1 >= 2:
        ...
        ret = f_a001_mig(a0, a1, m, N * np.exp(-a0 * t0),  N * np.exp(-a1 * t0), t)  / F_a001_mig(a0, a1, m, N * np.exp(-a0 * t0),  N * np.exp(-a1 * t0), t) * (1 - F_a001_coal(a0, N * np.exp(-a0 * t0), t)) ** (nC2(n1)) * (1 - F_a001_mig(a0, a1, m, N * np.exp(-a0 * t0),  N * np.exp(-a1 * t0), t)) ** (n1 - 1)
...
def main():
    ...
    for events in trees['E']:
        ...
        for e in events:
            ...
            if e[0] == 1:
                p_ = prob_a001_mig_(a1, a2, m, t, t_, n, pop_sizes[0], pop_sizes[1])
                ...
                _ = prob_a001_coal_(0, a1, a2, m, t, t_, n, pop_sizes[0], pop_sizes[1])
                logP += np.log2(_)
                ...
                _ = prob_a001_coal_(1, a1, a2, m, t, t_, n, pop_sizes[0], pop_sizes[1])
                logP += np.log2(_)
